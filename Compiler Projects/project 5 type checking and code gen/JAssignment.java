...
class JPlusAssignOp extends JAssignment {
    ...
    public JExpression analyze(Context context) {
        ...
        if (lhs.type().equals(Type.INT)) {
            ...
            type = Type.STRING;
        } else if (lhs.type().equals(Type.DOUBLE)) {  //double
            rhs.type().mustMatchExpected(line(), Type.DOUBLE);
            type = Type.DOUBLE;
        } else if (lhs.type().equals(Type.LONG)) {  //long
    ...
    public void codegen(CLEmitter output) {
        ...
        } else if (lhs.type().equals(Type.DOUBLE)) {  //double support
            ((JLhs) lhs).codegenLoadLhsRvalue(output);
            rhs.codegen(output);
            output.addNoArgInstruction(DADD);
        } else if (lhs.type().equals(Type.LONG)) {  //long support
            ((JLhs) lhs).codegenLoadLhsRvalue(output);
            rhs.codegen(output);
...
class JOrAssignOp extends JAssignment {
    ...
    public JExpression analyze(Context context) {
        ...
        rhs = (JExpression) rhs.analyze(context);
        if (lhs.type().equals(Type.INT)) {
            rhs.type().mustMatchExpected(line(), Type.INT);  //only int
            type = Type.INT;
        } else {
            JAST.compilationUnit.reportSemanticError(line(),
                    "Invalid lhs type for |=: " + lhs.type());
