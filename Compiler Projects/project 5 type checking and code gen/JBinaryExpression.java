...
class JMultiplyOp extends JBinaryExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        if (lhs.type() == Type.INT && rhs.type() == Type.INT) {
            type = Type.INT;
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE) {  //double
            type = Type.DOUBLE;
        } else if (lhs.type() == Type.LONG && rhs.type() == Type.LONG) {  //long
            type = Type.LONG;
        } else {
             type = Type.ANY;
...
class JOrOp extends JBinaryExpression {
     ...
     * {@inheritDoc}
     */
    public JExpression analyze(Context context) {  //lhs and rhs need to be ints
        lhs = (JExpression) lhs.analyze(context);
        rhs = (JExpression) rhs.analyze(context);
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IOR); //inclusive or opcode
    }
}
...
class JXorOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IXOR); //exclusive or opcode
    }
}
...
class JAndOp extends JBinaryExpression {
     ...
     */
    public JExpression analyze(Context context) {
        lhs = (JExpression) lhs.analyze(context);  //lhs and rhs need to be ints
        rhs = (JExpression) rhs.analyze(context);
        lhs.type().mustMatchExpected(line(), Type.INT);
        rhs.type().mustMatchExpected(line(), Type.INT);
        type = Type.INT;
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IAND); //and opcode
    }
}
...
class JALeftShiftOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(ISHL); //ALEFT_SHIFT opcode
    }
}
...
class JARightShiftOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(ISHR); //ARIGHT_SHIFT opcode
    }
}
...
class JLRightShiftOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IUSHR); //LRIGHT_SHIFT opcode
    }
}
