...
class JNegateOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        if (operand.type() == Type.INT) {
            type = Type.INT;
        } else if (operand.type() == Type.DOUBLE){  //double
            type = Type.DOUBLE;
        } else if (operand.type() == Type.LONG){  //long
            type = Type.LONG;
        } else {
            type = Type.ANY;
...
class JComplementOp extends JUnaryExpression {
     ...
     * {@inheritDoc}
     */
    public JExpression analyze(Context context) { //type checking for int
        operand = operand.analyze(context);
        operand.type().mustMatchExpected(line(), Type.INT);
        type = Type.INT;
        return this;
     ...
     * {@inheritDoc}
     */
    public void codegen(CLEmitter output) {    //this generates the bytecode
        operand.codegen(output);               //no bytecode for unary complement so
        output.addNoArgInstruction(ICONST_M1); //push int -1
        output.addNoArgInstruction(IXOR);      //we need to inverse bits and add 1
    }
}
...
class JPreDecrementOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        if (!(operand instanceof JLhs)) {
            ...
            operand = (JExpression) operand.analyze(context);
            operand.type().mustMatchExpected(line(), Type.INT);  //only int
            type = Type.INT;
        }
        return this;
    }
