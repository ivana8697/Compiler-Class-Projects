...
public class Parser {
    ...
    private JExpression assignmentExpression() {
        int line = scanner.token().line();
        JExpression lhs = inclusiveOrExpression(); //next
        if (have(ASSIGN)) {
    ...
    private JExpression andExpression() {
        int line = scanner.token().line();
        boolean more = true;
        JExpression lhs = shiftExpression(); //next
        while (more) {
            if (have(AND)) {
                lhs = new JAndOp(line, lhs, shiftExpression()); //next
            } else {
                more = false;
            }
        }
    ...
    //Parses shift expressions and returns an AST for it
    //shift expressions: ALEFT_SHIFT "<<" , ARIGHT_SHIFT ">>" ,
    //LRIGHT_SHIFT ">>>"
    private JExpression shiftExpression() {
        int line = scanner.token().line();
        JExpression lhs = conditionalAndExpression(); //next
        if (have(ALEFT_SHIFT)) {
            return new JALeftShiftOp(line, lhs, conditionalAndExpression());
        } else if (have(ARIGHT_SHIFT)) {
            return new JARightShiftOp(line, lhs, conditionalAndExpression());
        } else if (have(LRIGHT_SHIFT)) {
            return new JLRightShiftOp(line, lhs, conditionalAndExpression());
        } else {
            return lhs;
        }
    }
