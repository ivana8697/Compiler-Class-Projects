...
class Scanner {
    ...
    public TokenInfo getNextToken() {
        ...
        while (moreWhiteSpace) {
            ...
            if (ch == '/') {
                ...
                } else if (ch == '*'){ //indicates multi-line comment
                    nextCh();
                    while (ch != EOFCH){ //needs to keep looping through chars
                        nextCh();        //in comment until it reaches '*/' end
                        if(ch =='*'){
                            nextCh();
                            if (ch =='/'){
                                nextCh();
                                break;  //confirms end of comment
                            }           //so break while loop
                        }
                    }
                } else if (ch == '=') { //division assign
                    nextCh();
        ...
        switch (ch) {
            ...
            case '.':  //we need to use this with recognizing doubles
                nextCh();
                if (isDigit(ch)) {
                    ...
                    if (isExponential(ch)){   //account for exponential
                        buffer.append(ch);
                        nextCh();
                        if (ch == '+' || ch == '-') {
                            buffer.append(ch);
            ...
            case '>':
                nextCh();
                if (ch == '=') { //for ">=" greater than or equal to
                    nextCh();
                    return new TokenInfo(GTEQ, line);
                }
                if (ch == '>') {  //for ">>" arithmatic right shift
                    nextCh();
                    if (ch == '>') {  //for ">>>" logic right shift
                        nextCh();
                        if (ch == '=') { //for ">>>=" logic right shift assign
                            nextCh();
                            return new TokenInfo(LRIGHT_SHIFT_ASSIGN, line);
                        }
                        return new TokenInfo(LRIGHT_SHIFT, line);
                    }
                    if (ch == '=') { //for ">>=" arithmatic right assign
                        nextCh();
                        return new TokenInfo(ARIGHT_SHIFT_ASSIGN, line);
                    }
                    return new TokenInfo(ARIGHT_SHIFT, line);
                }
                return new TokenInfo(GT, line);  //for greater than
            case '<':
                nextCh();
                if (ch == '=') {
                    nextCh();
                ...
                if (ch == '<'){  //for "<<" arithmatic left shift
                    nextCh();
                    if (ch == '=') { //for "<<=" arithmatic left shift assign
                        nextCh();
                        return new TokenInfo(ALEFT_SHIFT_ASSIGN, line);
                    }
                    return new TokenInfo(ALEFT_SHIFT, line);
                }
            ...
            case '9':
                ...
                if (isLong(ch)){  //to process longs
                    buffer.append(ch);
                    nextCh();
                    return new TokenInfo(LONG_LITERAL, buffer.toString(), line);
                }
                if (isExponential(ch)){ //for exponents
                    buffer.append(ch);
                    nextCh();
                    if (ch == '+' || ch == '-') {
                        buffer.append(ch);
                    ...
                    if (isDouble(ch)) {  //if it has a suffix to indicate double
                        buffer.append(ch);
                        nextCh();
                    }
                    return new TokenInfo(DOUBLE_LITERAL, buffer.toString(), line);
                }
                if (isDouble(ch)){  //to process doubles
                    buffer.append(ch);
                    nextCh();
                    return new TokenInfo(DOUBLE_LITERAL, buffer.toString(), line);
                }
                if (ch == '.') {  //used for doubles
                    buffer.append(ch);
                    nextCh();
                    while (isDigit(ch)){
                        buffer.append(ch);
    ...
    // Returns true if the specified character is d or D, and false otherwise.
    private boolean isDouble(char c) {
        return c == 'd' || c == 'D';  //for double
    }
    // Returns true if the specified character is l or L, and false otherwise.
    private boolean isLong(char c) {
        return c == 'l' || c == 'L';  //for long
    }
    // Returns true if the specified character is e or E, and false otherwise.
    private boolean isExponential(char c) {
        return c == 'e' || c == 'E';  //indicates exponent
    }
